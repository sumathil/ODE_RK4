...
void odefun_gpu(float time_step, real *temp_sol, real *step_sol, int *sol_len, real *actual_sol)
{
	...
	real *d_Wx, *d_Wy, *d_GammaHV, *d_temp_sol, *d_step_1_sol, *d_step_2_sol, *d_step_3_sol, *d_step_4_sol,  *d_u, *d_v, *d_actual_sol;
	...
	cudaMalloc(&d_step_1_sol, nodes->internal_count * sizeof(real));
	cudaMalloc(&d_step_2_sol, nodes->internal_count * sizeof(real));
	cudaMalloc(&d_step_3_sol, nodes->internal_count * sizeof(real));
	cudaMalloc(&d_step_4_sol, nodes->internal_count * sizeof(real));
	/* cudaMalloc(&d_sol_len, nodes->total_count * sizeof(int)); */
	cudaMalloc(&d_actual_sol, nodes->total_count * sizeof(real));
	/* cudaMalloc(&d_actual_sol, nodes->total_count * sizeof(real)); */
	...
	/* cudaMemcpy(d_sol_len, sol_len, nodes->total_count * sizeof(int), cudaMemcpyHostToDevice); */
	cudaMemcpy(d_actual_sol, actual_sol, nodes->total_count * sizeof(real), cudaMemcpyHostToDevice);

	int threads_per_block = 256;
	cudaEventRecord(start,0);
	
	for (j = 0; j < *sol_len; j++)
			d_temp_sol[j] = d_actual_sol[j];
            
			update_ghost_nodes(d_temp_sol);
        	generate_u(time_step);
        	generate_v(time_step);		
			
			odefun_cuda<<<((nodes->internal_count - 1) / threads_per_block) + 1, threads_per_block>>>(d_Wx, d_Wy, d_GammaHV, d_neighbors, nodes->internal_count, stencil_size, d_u, d_v, d_temp_sol, d_step_1_sol);
			d_factor = (real)(time_step / 2);
			cudaDeviceSynchronize();
			update_solution(d_temp_sol, d_actual_sol, d_step_1_sol, d_factor, sol_len);
			update_ghost_nodes(d_temp_sol);
			generate_u(time_step);
	        generate_v(time_step);
			odefun_cuda<<<((nodes->internal_count - 1) / threads_per_block) + 1, threads_per_block>>>(d_Wx, d_Wy, d_GammaHV, d_neighbors, nodes->internal_count, stencil_size, d_u, d_v, d_temp_sol, d_step_2_sol);
			update_solution(d_temp_sol, d_actual_sol, d_step_2_sol, d_factor, sol_len);
			update_ghost_nodes(d_temp_sol);
			cudaDeviceSynchronize();
			generate_u(time_step);
        	generate_v(time_step);
			odefun_cuda<<<((nodes->internal_count - 1) / threads_per_block) + 1, threads_per_block>>>(d_Wx, d_Wy, d_GammaHV, d_neighbors, nodes->internal_count, stencil_size, d_u, d_v, d_temp_sol, d_step_3_sol);
			d_factor = time_step;
			cudaDeviceSynchronize();
			update_solution(d_temp_sol, d_actual_sol, d_step_3_sol, d_factor, sol_len);
			update_ghost_nodes(d_temp_sol);
       		generate_u(time_step);
        	generate_v(time_step);
			odefun_cuda<<<((nodes->internal_count - 1) / threads_per_block) + 1, threads_per_block>>>(d_Wx, d_Wy, d_GammaHV, d_neighbors, nodes->internal_count, stencil_size, d_u, d_v, d_temp_sol, d_step_4_sol);
			d_factor = (real)(time_step / 6);
			cudaDeviceSynchronize();
	for (j = 0; j < *sol_len; j++)
			d_actual_sol[j] = d_actual_sol[j] + d_factor * (d_step_1_sol[j] + (2 * d_step_2_sol[j]) + (2 * d_step_3_sol[j]) + d_step_4_sol[j]);
	cudaDeviceSynchronize();
	...
	cudaEventRecord(start,0);
	cudaMemcpy(actual_sol, d_actual_sol, nodes->internal_count * sizeof(real), cudaMemcpyDeviceToHost);
        cudaEventRecord(stop,0);
        cudaEventSynchronize(stop);
        cudaEventElapsedTime(&elapsedTime,start,stop);
        printf("Elapsed Time = %f\n",elapsedTime);
	...
	cudaFree(d_GammaHV);
	cudaFree(d_temp_sol);
	cudaFree(d_step_1_sol);
	cudaFree(d_step_2_sol);
	cudaFree(d_step_3_sol);
	cudaFree(d_step_4_sol);
	cudaFree(d_u);
	cudaFree(d_v);
